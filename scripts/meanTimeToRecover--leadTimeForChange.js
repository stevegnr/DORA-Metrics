...
async function timeToFixBugs(cloudId, startAt = 0, maxResults = 100) {
  ...
  for (let i = 0; i < issues.issues.length; i++) {
    ...
    const changelogOfAnIssue = changelog(issue.key);
    let dateCodeReview;
    let dateMergedInProd;
    let timeForChangeMs;
    let timeForChangeDays = timeForChangeMs / toDays;
    ...
    timeForChangeDays = timeForChangeMs / toDays;

    console.log(Math.round(timeForChangeDays * 100) / 100);
...
async function leadTimeForChange() {
  const changelogOfAnIssue = await changelog();
  let dateCodeReview;
  let dateMergedInProd;
  let timeForChangeMs;
  let timeForChangeDays = timeForChangeMs / toDays;
